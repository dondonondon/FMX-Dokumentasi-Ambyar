...
function fnGetLocation : String; overload;
function fnGetLocation(AFolder : String) : String; overload;
function fnCheckFile(nmFile : String) : Boolean; overload;
function fnCheckFile(nmFile, AFolder : String) : Boolean; overload;
...
procedure fnSaveINI(Section, Name, Value: string); overload;
function fnLoadINI(Section, Name: string): string; overload;

procedure fnSaveINI(AFolder, AFile, Section, Name, Value: string); overload;
function fnLoadINI(AFolder, AFile, Section, Name: string): string; overload;
...
function fnGetLocation(AFolder : String) : String;
begin
  {$IF DEFINED(IOS) or DEFINED(ANDROID)}
    Result := TPath.GetDocumentsPath + PathDelim;
  {$ELSEIF Defined(MSWINDOWS)}
...
function fnCheckFile(nmFile, AFolder : String) : Boolean;
var
  loc : String;
begin
  loc := fnGetLocation(AFolder) + nmFile;
...
procedure fnSaveINI(Section, Name, Value: string);
var
  ini: TIniFile;
  loc : String;
begin
  loc := fnGetLocation('log');
  ini := TIniFile.Create(loc + 'log.ini');
  try
    ini.WriteString(Section, Name, Value);
  finally
    ini.DisposeOf;
...
function fnLoadINI(Section, Name: string): string;
var
  ini: TIniFile;
  loc : String;
begin
...
procedure fnSaveINI(AFolder, AFile, Section, Name, Value: string);
var
  ini: TIniFile;
  loc : String;
begin
  loc := fnGetLocation(AFolder);
  ini := TIniFile.Create(loc + AFile);
  try
    ini.WriteString(Section, Name, Value);
  finally
    ini.DisposeOf;
...
function fnLoadINI(AFolder, AFile, Section, Name: string): string;
var
  ini: TIniFile;
  loc : String;
begin
...
begin
  ...
  //AParent.Anchors := [TAnchorKind.akLeft, TAnchorKind.akTop, TAnchorKind.akRight, TAnchorKind.akBottom];
end;
