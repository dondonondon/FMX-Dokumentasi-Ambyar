...
procedure fnVisibleFooter;
procedure fnSetFooter(btn : TCornerButton);
...
procedure fnGetE(str : String); overload;
...
begin
  ...
  if goFrame <> LOADING then
    TAndroidHelper.Activity.getWindow.setNavigationBarColor($FFF3F4F9);
  {$ENDIF}

  with FMain do begin
    if (goFrame = HOME) or (goFrame = LOGIN) or (goFrame = FEED) then begin
      vsMain.Height := FMain.Height - loFooter.Height;
      loFooter.Visible := True;
    end else begin
      vsMain.Height := FMain.Height;
      loFooter.Visible := False;
    end;
  end;
end;
...
var
  sect : String;
begin
  sect := FormatDateTime('YYYY-MM-DD HH:NN:SS', Now);

  fnSaveINI('log', 'error.txt', sect, 'Message', Msg);
  fnSaveINI('log', 'error.txt', sect, 'Class', Cls);
end;

procedure fnGetE(str : String);
var
  sect : String;
begin
  sect := FormatDateTime('YYYY-MM-DD HH:NN:SS', Now);
  fnSaveINI('log', 'check.txt', sect, 'Message', str);
end;
